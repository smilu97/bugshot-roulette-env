...
class DefaultBugShotStateSelector(BugShotStateSelector):
    ...
    def select(self, state: BugShotState) -> list[int]:
        ...
        is_magnified_live = int(state.is_magnified_shell and (state.chamber[-1] == BugShotShell.LIVE))
        is_magnified_blank = int(state.is_magnified_shell and (state.chamber[-1] == BugShotShell.BLANK))
